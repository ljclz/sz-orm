//! 内存实现：线性扫描 + 子串匹配（不连接真实搜索引擎，无倒排索引）
//!
//! 适用于单元测试和基准场景。支持：
//! - 全文搜索（基于 JSON 文档字符串的子串匹配，O(n) 扫描）
//! - 字段过滤（term 精确匹配）
//! - 排序（数值/字符串）
//! - 分页
//!
//! 注意：本实现没有倒排索引、分词器、posting list 等真实搜索引擎的核心组件，
//! 仅适用于小数据量测试场景；生产环境请启用 `real-es`/`real-opensearch`/`real-meilisearch` feature。
