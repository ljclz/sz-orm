...
/// 创建连接池
#[no_mangle]
pub extern "C" fn sz_orm_cpp_pool_new(config: PoolConfigC) -> SzOrmPoolHandle {
    let _ = config;
    std::ptr::null_mut()
}
...
/// 执行查询
#[no_mangle]
pub extern "C" fn sz_orm_cpp_query(
...
mod tests {
    ...
    fn test_pool_new_returns_null() {
        let handle = sz_orm_cpp_pool_new(PoolConfigC::default());
        assert!(handle.is_null());
    }

    #[test]
    fn test_pool_free_null_is_safe() {
        unsafe { sz_orm_cpp_pool_free(std::ptr::null_mut()) };
    }

    #[test]
    fn test_query_null_returns_error() {
        let result = sz_orm_cpp_query(std::ptr::null_mut(), std::ptr::null());
    ...
    #[test]
    fn test_version() {
        assert_eq!(sz_orm_cpp_version(), 1);
    }
}
