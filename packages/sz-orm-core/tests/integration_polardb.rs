//! PolarDB 兼容方言集成测试
//!
//! PolarDbDialect 通过 delegate_dialect_to! 委派 PostgreSqlDialect，
//! 生成的 SQL 与 PostgreSqlDialect 一致。用 PostgreSQL 18 验证兼容性。
//!
//! 运行方式：cargo test -p sz-orm-core --test integration_polardb -- --ignored --nocapture
...
#[test]
fn test_polardb_dialect_quote() {
    let polardb = get_dialect(DbType::PolarDB).unwrap();
    let pg = get_dialect(DbType::PostgreSQL).unwrap();
    assert_eq!(polardb.quote("user"), pg.quote("user"));
    assert_eq!(polardb.quote("user"), "\"user\"");
}

#[test]
fn test_polardb_dialect_escape() {
    let polardb = get_dialect(DbType::PolarDB).unwrap();
    let pg = get_dialect(DbType::PostgreSQL).unwrap();
    assert_eq!(polardb.escape_string("it's"), pg.escape_string("it's"));
}

#[test]
fn test_polardb_dialect_pagination() {
    let polardb = get_dialect(DbType::PolarDB).unwrap();
    let pg = get_dialect(DbType::PostgreSQL).unwrap();
    assert_eq!(
        polardb.build_pagination("SELECT 1", 10, 20),
        pg.build_pagination("SELECT 1", 10, 20)
    );
}

#[test]
fn test_polardb_dialect_db_type() {
    let dialect = get_dialect(DbType::PolarDB).unwrap();
    assert_eq!(dialect.db_type(), DbType::PolarDB);
}

#[test]
fn test_polardb_dialect_create_table() {
    let polardb = get_dialect(DbType::PolarDB).unwrap();
    let pg = get_dialect(DbType::PostgreSQL).unwrap();
    let sql_polardb = polardb.build_create_table("test_table", &test_columns());
    let sql_pg = pg.build_create_table("test_table", &test_columns());
    assert_eq!(sql_polardb, sql_pg);
}
...
#[tokio::test]
#[ignore]
async fn test_polardb_crud_on_pg() {
    let pool = setup_pool().await;
    let dialect = get_dialect(DbType::PolarDB).unwrap();
    let table = unique_table("polardb");
...
#[tokio::test]
#[ignore]
async fn test_polardb_pagination_on_pg() {
    let pool = setup_pool().await;
    let dialect = get_dialect(DbType::PolarDB).unwrap();
    let table = unique_table("polardb_pg");
...
#[tokio::test]
#[ignore]
async fn test_polardb_sql_injection_protection() {
    let pool = setup_pool().await;
    let dialect = get_dialect(DbType::PolarDB).unwrap();
    let table = unique_table("polardb_inj");
