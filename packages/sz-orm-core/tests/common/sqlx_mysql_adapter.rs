//! sqlx MySQL 到 sz_orm_core::Connection 的适配器
...
pub struct SqlxMySqlAdapter {
    pool: MySqlPool,
}

impl SqlxMySqlAdapter {
    ...
    fn try_get_value(row: &MySqlRow, idx: usize) -> Value {
...
impl Connection for SqlxMySqlAdapter {
    fn execute<'a>(
        &'a mut self,
        sql: &'a str,
