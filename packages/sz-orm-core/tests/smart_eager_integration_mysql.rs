//! SmartEagerLoader MySQL 方言集成测试（v2.4.0 任务 2.1）
//!
...
use common::sqlx_mysql_adapter::SqlxMySqlAdapter;
...
async fn teardown_mysql(conn: &mut SqlxMySqlAdapter) {
    let builder = TestSchemaBuilder::new(TestDialect::MySql);
...
#[tokio::test]
#[ignore]
async fn test_join_strategy_mysql() {
    let relation = RelationDef::new(
        "profile",
        "users",
        "profiles",
...
#[tokio::test]
#[ignore]
async fn test_dataloader_strategy_mysql() {
    let relation = RelationDef::new(
        "orders",
        "users",
        "orders",
...
#[tokio::test]
#[ignore]
async fn test_intermediate_strategy_mysql() {
    let relation = RelationDef::new_many_to_many(
        "roles",
        "users",
        "roles",
