...
/// SAFETY: handle must be a valid handle returned by `sz_orm_go_pool_new`.
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_pool_stats(handle: SzOrmPoolHandle) -> PoolStatsC {
    // SAFETY: 转发到 cabi，handle 有效性由调用方保证
...
/// SAFETY: handle must be a valid handle returned by `sz_orm_go_pool_new`.
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_pool_metrics(handle: SzOrmPoolHandle) -> PoolMetricsC {
    // SAFETY: 转发到 cabi，handle 有效性由调用方保证
...
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_query_one(
    handle: SzOrmPoolHandle,
    sql: *const c_char,
) -> *mut QueryJsonResult {
...
/// SAFETY: handle must be valid; sql must be a valid NUL-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_execute_batch(
    handle: SzOrmPoolHandle,
    sql: *const c_char,
) -> QueryResultC {
    // SAFETY: 转发到 cabi，参数有效性由调用方保证
...
/// # Safety
///
/// SAFETY: result must be a valid pointer returned by `sz_orm_go_query_one`.
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_query_result_free(result: *mut QueryJsonResult) {
...
/// # Safety
///
/// SAFETY: Caller must free the returned string with `sz_orm_go_string_free`.
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_error_description(code: i32) -> *mut c_char {
    // SAFETY: 转发到 cabi
...
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_table_exists(
    handle: SzOrmPoolHandle,
    table: *const c_char,
) -> i32 {
    // SAFETY: 转发到 cabi，参数有效性由调用方保证
...
/// SAFETY: handle must be valid; table must be a valid NUL-terminated C string.
#[no_mangle]
pub unsafe extern "C" fn sz_orm_go_count(handle: SzOrmPoolHandle, table: *const c_char) -> i64 {
    // SAFETY: 转发到 cabi，参数有效性由调用方保证
