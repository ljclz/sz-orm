# sz-orm v4.3.0 开发规划（查询智能与开发者体验）

> 版本：v4.3.0（编译期 EXPLAIN 分析 + 查询性能火焰图 + N+1 静态检测 + 数据血缘可视化 + 编译期数据治理 + 自适应查询 + 多数据库融合（可选））
> 基线：v4.2.0（跨语言分布式事务 + Go/Java/C++ 绑定 + 可视化 Schema 设计器 + OpenAPI → ORM 反向生成 + WASM 真实数据库连接，40 任务 / 278 子任务 / 9.5 周，7 个 feature gate 隔离）
> 日期：2026-08-12
> 文档定位：开发规划（What/How/When），与 v4.2.0（`docs/spec/v4.2.0/`）边界清晰，无任务重叠
> 规划依据：`docs/评估/` 8 份评估报告 + 2026-08-12 逐项代码验证（file:line 均已实测存在）
...
> 测试命令：`cargo test --workspace -j 2 --no-fail-fast`；feature 包测试：`cargo test -p <package> --features <feature>`

---

# 一、规划背景与依据

## 1.1 评估结论摘要（2026-08-12 实测验证）

对 `docs/评估/` 8 份文档逐项对照实际代码验证，结论如下：

| 评估声明 | 验证结果 | 证据 |
|---------|---------|------|
| 28 种方言（21 默认 + 7 门控） | ✅ 精确 | `packages/sz-orm-core/src/db_type.rs:11`（28 变体，`:56-74` 门控） |
| 17 个派生宏（11 derive + 6 proc_macro） | ✅ 精确 | `packages/sz-orm-macros/src/lib.rs`（实测 11 + 6） |
| v4.0.0 9 项新增能力 | ✅ 真实，行号/LOC 精确 | `sz-orm-ai/src/llm_provider/router.rs:27`(206L) 等 9 处 |
| 核心 API file:line 证据（25+ 处） | ✅ 基本全部命中 | `query.rs:36/596/986/254`、`pool.rs:1940` 等 |
| 编译期 SQL 验证（db-verify） | ✅ 已存在（被低估） | `macros/lib.rs:546-588` 已支持编译期连真 DB 执行 EXPLAIN |
| 每项 v4.0.0 能力测试数（213/275/161…） | ❌ 虚构（实际 23/18/52…，个别超整包总数） | 实测各功能包 |
| "88 个 ZST 结构" | ❌ 查无实据 | `typed_ast.rs` 实测 24 个结构体 |
| "crates.io 仅发布 1/46 包" | ❌ 重大低估 | registry 实测 58 个版本条目、20+ 个包已发布（1.x~3.x 版本线） |
| LOC 265,695 / 测试 6,623 | ⚠️ 低估 10~22% | 实测 291,349 LOC / 6,794 `#[test]` + 781 `#[tokio::test]` + 1,320 集成 |

## 1.2 本规划选型原则

1. **只选已验证存在代码基础的方向**（复用优先，杜绝"自嗨"）
2. **排除需重构核心架构的方向**：编译期数据库、类型级查询验证（`QueryBuilder` 运行时构造 `query.rs:36`、`Model::table_name()` 运行时返回 `model.rs:52`，前移编译期 = 重写 ORM，与"无 Breaking Change + sz-pay 生产依赖不破坏"铁律冲突）
3. **与 v4.2.0 正在开发的内容零重叠**（见第二章边界声明）
4. **数量声明一律以实测为准**（本规划所有 file:line 已实测，不再引用评估文档中的虚构数字）

---

# 二、与 v4.2.0 的边界声明（不冲突保证）

## 2.1 v4.2.0 已占用范围（本规划不触碰）

| v4.2.0 里程碑 | 占用包/模块 | feature gate |
|--------------|------------|--------------|
| M1 跨语言分布式事务 | `sz-orm-dtx`（cross_lang 模块 + lib.rs 扩展） | `cross-lang-dtx` |
| M2 Go/Java/C++ 绑定 | `sz-orm-cabi` / `sz-orm-go` / `sz-orm-java` / `sz-orm-cpp`（新包）+ `sz-orm-core` 只读复用 | `lang-binding-*` |
| M3 Schema 设计器 | `sz-orm-designer`（新包） | `schema-designer` |
| M3 OpenAPI 反向生成 | `sz-orm-swagger`（reverse 模块） | `openapi-reverse` |
| M4 WASM 真实 DB | `sz-orm-wasm`（real_db 模块） | `wasm-real-db` |
| 最终验证 | `cli/src/main.rs`（designer/openapi 命令） | — |

## 2.2 v4.3.0 新增范围（全部新包或 v4.2.0 未触碰的既有包）

| 里程碑 | 新增包/扩展包 | 与 v4.2.0 关系 |
|--------|--------------|---------------|
| M1 | `sz-orm-explain`（新包）+ `sz-orm-macros`（扩展 db-verify，v4.2.0 不触碰） | 零重叠 |
| M1 | `sz-orm-flamegraph`（新包）+ `sz-orm-tracing`（只读复用 Tracer） | 零重叠（建议 v4.2.0 M1 完成后启动，避免 merge 冲突） |
| M2 | `sz-orm-n1-lint`（新包）+ `cli`（新增命令，与 v4.2.0 CLI 改动文件相同但命令名不同，**串行**） | 需注意：cli/src/main.rs 与 v4.2.0 M3-T7/M3-T13 同文件，**必须 v4.2.0 交付后启动** |
| M3 | `sz-orm-audit`（lineage-viz 扩展，v4.2.0 不触碰）+ `sz-orm-masking`（只读复用） | 零重叠 |
| M4 | `sz-orm-adaptive`（新包，依赖 sz-orm-core 只读 + sz-orm-ai 只读） | 零重叠 |
| M5（可选） | `sz-orm-fusion`（新包） | 零重叠 |

**冲突规避规则**：
- 唯一同文件风险：`cli/src/main.rs`（v4.2.0 M3-T7/M3-T13 与 v4.3.0 M2-T4 均修改此文件）→ M2 排期在 v4.2.0 交付之后
- 唯一同包扩展：`sz-orm-macros`（v4.2.0 不修改此包，仅 M3-T9 使用 syn/quote 依赖模式参考）→ 安全
- 其余里程碑全部落在新包或 v4.2.0 不触碰的包，可并行

## 2.3 并行排期建议

```
2026-08-11 ── v4.2.0（M1→M2→M3→M4，9.5 周）──────────────────────────→ ~10 月中
              │
              ├─ M0（纯文档，立即并行，0.5 周）
              ├─ M1（sz-orm-explain 新包，可与 v4.2.0 并行；flamegraph 建议 v4.2.0 M1 后）
              ├─ M3（sz-orm-audit，可与 v4.2.0 并行）
              └─ M2 / M4 / M5 ── 等 v4.2.0 交付后启动（cli 同文件 + 依赖 core 最终形态）
```
...
| 里程碑 | 名称 | 优先级 | 任务数 | 子任务数 | 预期工作量 | 启动时机 |
|--------|------|--------|--------|----------|-----------|---------|
| M0 | 评估文档修正与基线合并 | P0 | 3 | 12 | 0.5 周 | 立即（与 v4.2.0 并行） |
| M1 | 查询智能：编译期 EXPLAIN 分析 + 火焰图 | P1 | 6 | 42 | 2.5 周 | 与 v4.2.0 并行 |
| M2 | N+1 静态检测器 | P1 | 3 | 18 | 2 周 | v4.2.0 交付后 |
| M3 | 数据治理：血缘可视化 + 编译期治理 | P2 | 5 | 30 | 2.5 周 | 与 v4.2.0 并行 |
| M4 | 自适应查询优化器 | P2 | 3 | 18 | 2 周 | v4.2.0 交付后 |
| M5 | 多数据库融合查询（可选/实验） | P3 | 3 | 16 | 3 周 | v4.2.0 交付后 |
| M6 | 最终验证与文档同步 | 全局 | 3 | 16 | 0.5 周 | 全部完成后 |
| **合计** | — | — | **26** | **152** | **13 周**（不含 M5 为 10 周） | — |

## 3.2 任务编号约定

- 主任务：`M{里程碑}-T{序号}`（如 M1-T1）；子任务：`M1-T1.{序号}`
- 每个里程碑末尾固定一个集成测试与门禁验证任务
...
5. **五方言覆盖**：MySQL/PostgreSQL/SQLite/Oracle/MSSQL（EXPLAIN 解析/治理规则/融合查询按方言能力适配）
6. **审计证据**：每项任务结论附真实存在的 file:line 证据
7. **测试基线不回退**：v4.2.0 已验收测试基线不回退，v4.3.0 仅增不减
8. **复用优先**：不重复实现（EXPLAIN 解析复用 `query!` 宏 db-verify 模式；火焰图复用 `Tracer`/`SzTracer`；N+1 检测复用 `N1QueryDetector` 运行时知识；血缘复用 `LineageGraph`；治理复用 `access_control.rs`/`sz-orm-masking`；自适应复用 `plan_cache.rs`/`l1_cache.rs`/`l2_cache.rs`/`paginator.rs`）
9. **Windows MSVC 编译环境**：RUST_MIN_STACK=134217728, CARGO_INCREMENTAL=0

## 3.4 feature gate 定义

| feature gate | 所属包 | 控制能力 | 默认 |
|-------------|--------|---------|------|
...
| `db-fusion` | sz-orm-fusion（新包，可选） | 多数据库融合查询（拆分 + 聚合 + 同步） | 关闭 |

---

# 四、M0：评估文档修正与基线合并（P0，0.5 周）
...
- [ ] M0-T1.7 标注"编译期执行计划分析 ❌ 无"为错误结论（db-verify 已支持 EXPLAIN）
...
- [ ] M0-T3.1 运行 `python scripts/check-doc-consistency.py`（门禁 12）
- [ ] M0-T3.2 运行 `bash scripts/audit-verify.sh 2026-08-12_v4.2.0_基线评估.md`（门禁 13）
- [ ] M0-T3.3 运行 `python scripts/check-doc-sync.py --diff HEAD`（门禁 14）

**验收标准**：
1. 8 份评估文档中的虚构数字全部替换为实测值（附 file:line 证据）
2. 基线评估报告全部结论可被 `audit-verify.sh` 验证通过
3. 文档一致性/同步检查通过
...
# 五、M1：查询智能 — 编译期 EXPLAIN 分析 + 火焰图（P1，2.5 周）
...
**任务描述**：新增 `sz-orm-explain` 包（EXPLAIN 结果解析器，5 方言），`explain-analyzer` feature gate 隔离。
...
**复用标注**：既有 28 方言枚举 `db_type.rs:11`；`validate_sql_content`（`packages/sz-orm-macros/src/lib.rs:298`）模式参考
...
- [ ] M1-T1.4 验证 `cargo check -p sz-orm-explain` 编译通过
...
**任务描述**：实现 MySQL/PostgreSQL/SQLite/Oracle/MSSQL 的 EXPLAIN 输出解析为统一 `ExplainPlan`。
...
**涉及文件**：
- `packages/sz-orm-macros/src/lib.rs`（修改：`verify_with_db` 函数扩展，`:546-588` 区域）
- `packages/sz-orm-macros/Cargo.toml`（`db-verify` feature 增加 `sz-orm-explain` 依赖）

**复用标注**：既有 db-verify EXPLAIN 执行链路 `macros/lib.rs:546-588`；`SZ_ORM_QUERY_VERIFY` 环境变量开关 `:548`

**子任务**：
- [ ] M1-T3.1 `db-verify` feature 增加可选依赖 `sz-orm-explain`
- [ ] M1-T3.2 在 db-verify EXPLAIN 执行后（`:546` 区域），调用 `sz-orm-explain` 解析结果
...
- [ ] M1-T3.5 警告不阻断编译（仅 warning），解析失败降级为无警告
- [ ] M1-T3.6 单元测试：mock EXPLAIN 输出（Seq Scan），编译期输出 warning 文案
- [ ] M1-T3.7 集成验证：`SZ_ORM_QUERY_VERIFY=1 DATABASE_URL=mysql://...` 连真库编译，验证全表扫描警告

**验收标准**：`query!` 宏在 db-verify 模式下输出编译期性能警告（全表扫描/缺失索引）；默认模式行为不变；警告不阻断编译

**依赖**：M1-T1、M1-T2

## M1-T4：执行计划回归检测（CI 集成）

**任务描述**：提供 `ExplainPlan` 基线快照与回归对比，CI 中检测查询性能退化（如 IndexRange 变为 FullTable）。
...
- [ ] M1-T4.2 实现 `PlanSnapshot::to_json()/from_json()`（基线文件格式）
...
- [ ] M1-T4.5 单元测试：IndexRange → FullTable 检出 `ScanTypeUpgrade`；索引丢失检出 `IndexLost`
- [ ] M1-T4.6 集成测试：生成基线快照 → 修改查询 → 对比检出回归

**验收标准**：基线快照 + 回归对比 + CI 入口；检出逻辑测试通过
...
**复用标注**：既有 `Tracer` trait `packages/sz-orm-tracing/src/lib.rs:129`、`SzTracer` `:136`（span 关联复用）；既有 stream/查询执行链路（`query.rs:36` QueryBuilder、`pool.rs:45` Connection）

**子任务**：
- [ ] M1-T5.1 创建 `sz-orm-flamegraph` 包，`[features] query-flamegraph`，默认关闭
...
- [ ] M1-T5.4 实现 `QueryTracer::with_tracer(tracer: &dyn Tracer)`：将阶段耗时写入既有 Tracer span（复用 `:129`）
...
**子任务**：
- [ ] M1-T6.1 集成测试：`query!` 宏 + db-verify + 真库，全表扫描警告触发（MySQL/PostgreSQL）
...
- [ ] M1-T6.5 扫描 `grep -rn 'todo!\|unimplemented!\|unreachable!' packages/sz-orm-explain/ packages/sz-orm-flamegraph/` 无占位
- [ ] M1-T6.6 验证默认 feature 行为与 v4.2.0 一致（不启用新 feature）

**验收标准**：M1 集成测试通过；clippy/fmt/占位检查通过；默认 feature 行为不变

**依赖**：M1-T1~M1-T5

---

# 六、M2：N+1 静态检测器（P1，2 周）
...
**涉及文件**：
- `packages/sz-orm-n1-lint/Cargo.toml`（新建，依赖 syn/quote/proc-macro2 + sz-orm-core）
- `packages/sz-orm-n1-lint/src/lib.rs`（新建）
- `Cargo.toml`（workspace.members 新增）

**子任务**：
- [ ] M2-T1.1 创建 `sz-orm-n1-lint` 包（cdylib 无需，纯 lib + bin）
- [ ] M2-T1.2 `[features] n1-lint`，默认关闭；依赖 `syn = { version = "2.0", features = ["full"] }`
- [ ] M2-T1.3 定义 `#[proc_macro_attribute] pub fn detect_n_plus_one(_attr, item) -> TokenStream`：解析函数体 → 分析 → 生成警告 + 透传原函数
...
- [ ] M2-T1.7 检测模式三：循环体内出现 `where_in` 可批量替代的单查询（引用 `eager_loader.rs`/`find_with_related.rs` 既有能力）→ `MissingEagerLoadHint`
- [ ] M2-T1.8 生成 `proc_macro::Span::warning` 或 `compile_error!`（默认 warning，`#![allow(n_plus_one)]` 可抑制）

**验收标准**：标注宏可解析函数体 AST；3 种检测模式单元测试通过；默认 warning 不阻断编译

**依赖**：无

## M2-T2：CLI 集成 + 批量检测

**涉及文件**：`cli/src/main.rs`（修改，**须在 v4.2.0 交付后**）、`packages/sz-orm-n1-lint/src/batch.rs`（新建）
...
- [ ] M2-T2.4 JSON 输出格式（`serde_json` 序列化 findings）
...
**验收标准**：CLI 命令可用；批量扫描检出循环内查询；JSON 输出可被 CI 消费
...
**子任务**：
- [ ] M2-T3.1 集成测试：既有 `N1QueryDetector`（`entity_graph.rs:641`）与静态检测结果交叉验证（同一样例代码，运行时检测与静态检测发现一致）
- [ ] M2-T3.2 运行 `cargo test -p sz-orm-n1-lint --features n1-lint`
- [ ] M2-T3.3 `cargo clippy -p sz-orm-n1-lint --features n1-lint -- -D warnings`
- [ ] M2-T3.4 扫描 `grep -rn 'todo!\|unimplemented!\|unreachable!' packages/sz-orm-n1-lint/` 无占位
- [ ] M2-T3.5 验证默认 feature 行为不变

**验收标准**：静态/运行时检测交叉一致；门禁通过；默认行为不变

**依赖**：M2-T1、M2-T2

---

# 七、M3：数据治理 — 血缘可视化 + 编译期治理（P2，2.5 周）
...
**复用标注**：既有 `LineageGraph` `graph.rs:96`、`LineageExporter` `export.rs`（`:12` 区域）
...
- [ ] M3-T1.6 单元测试：Graphviz 输出可被 `dot` 语法解析（结构断言）

**验收标准**：3 种格式导出；链路完整性测试通过；`cargo test -p sz-orm-audit --features lineage-viz`
...
**子任务**：
- [ ] M3-T2.1 实现 `pub fn downstream_impact(graph: &LineageGraph, node: &str, depth: usize) -> Vec<ImpactEdge>`（变更影响范围）
...
- [ ] M3-T2.5 单元测试：深度限制（depth=1 只返回直接下游）
- [ ] M3-T2.6 集成测试：迁移 dry-run（`migration_dry_run.rs:94`）与血缘影响分析联动，DROP 前输出受影响链路

**验收标准**：影响分析正确（BFS 深度受限）；与迁移 dry-run 联动测试通过

**依赖**：M3-T1

## M3-T3：compile-governance feature + PII 标注强制

**任务描述**：在 `sz-orm-core` 增加 `compile-governance` feature：`#[pii]` 字段标注 + 编译期强制（PII 字段必须经过脱敏/授权访问路径），生成合规报告。
...
**复用标注**：既有 `access_control.rs`（`packages/sz-orm-core/src/access_control.rs`，ABAC 权限）、`sz-orm-masking`（脱敏规则）、既有 derive 宏模式 `macros/lib.rs:2853`
...
- [ ] M3-T3.5 生成运行时治理代码：`fn pii_fields() -> Vec<&'static str>` + `fn mask_policy(field) -> Option<MaskPolicy>`（复用 sz-orm-masking 执行）
- [ ] M3-T3.6 单元测试：合法标注编译通过；缺 mask 的 PII 字段编译失败
- [ ] M3-T3.7 单元测试：非法 mask 策略编译失败
- [ ] M3-T3.8 集成测试：`Governed` 模型通过既有 `access_control.rs` 检查后，PII 字段输出自动脱敏

**验收标准**：编译期强制（缺 mask/非法策略 → compile_error）；运行时脱敏复用 sz-orm-masking；默认 feature 行为不变
...
- [ ] M3-T4.5 集成测试：与既有 `sz-orm-audit` 审计链联动（报告哈希入链）

**验收标准**：合规报告生成正确；JSON 输出；审计联动测试通过
...
- [ ] M3-T5.3 扫描占位实现
- [ ] M3-T5.4 验证默认 feature 行为与 v4.2.0 一致
- [ ] M3-T5.5 验证 `compile-governance` 与既有 feature 组合编译（`--all-features`）

**验收标准**：门禁通过；feature 组合编译通过；默认行为不变

**依赖**：M3-T1~M3-T4

---

# 八、M4：自适应查询优化器（P2，2 周）
...
**子任务**：
- [ ] M4-T1.1 创建 `sz-orm-adaptive` 包
- [ ] M4-T1.2 `[features] adaptive-query`，默认关闭
- [ ] M4-T1.3 定义 `pub struct QueryStats { total_executions: AtomicU64, total_rows: AtomicU64, total_time_us: AtomicU64 }` + `record(&self, rows: u64, time_us: u64)`
...
**复用标注**：既有游标分页 `cursor_stream.rs`、`paginator.rs`（`packages/sz-orm-core/src/`）、L1/L2 缓存（`l1_cache.rs`/`l2_cache.rs`）
...
- [ ] M4-T2.3 自动游标分页：`should_paginate` 为真时切换到既有游标分页（复用 `cursor_stream.rs`），返回分页句柄而非全量
- [ ] M4-T2.4 自动缓存：`should_cache` 为真时结果写入既有 L2 缓存（复用 `l2_cache.rs`），TTL 按 `cache_time_ms`
- [ ] M4-T2.5 慢查询降级：单次执行超时（`timeout_ms` 可配）返回明确超时错误，不静默
- [ ] M4-T2.6 单元测试：模拟统计增长，验证 `should_paginate` 状态翻转
- [ ] M4-T2.7 单元测试：缓存命中返回缓存结果，统计不重复累加执行时间
- [ ] M4-T2.8 性能测试：统计采集开销 < 1μs/次（原子操作，无锁）

**验收标准**：决策执行器完整；复用既有缓存/分页实现（不重写）；性能测试通过
...
- [ ] M4-T3.2 运行 `cargo test -p sz-orm-adaptive --features adaptive-query`
- [ ] M4-T3.3 clippy/fmt/占位检查
- [ ] M4-T3.4 验证默认 feature 行为不变

**验收标准**：集成测试通过；门禁通过；默认行为不变

**依赖**：M4-T1、M4-T2

---

# 九、M5：多数据库融合查询（P3，可选/实验，3 周）
...
**子任务**：
- [ ] M5-T1.1 创建 `sz-orm-fusion` 包，`[features] db-fusion`，默认关闭
- [ ] M5-T1.2 定义 `pub struct FusionConfig { pub primary: Dialect, pub cache: Option<CacheBackend>, pub search: Option<SearchBackend> }`
- [ ] M5-T1.3 定义 `pub enum CacheBackend { Redis }` / `pub enum SearchBackend { Vector }`（复用既有 `sz-orm-vector` 混合搜索 `searcher.rs:30`）

## M5-T2：查询拆分与聚合

**子任务**：
- [ ] M5-T2.1 实现 `FusionPlanner::plan(query: &QueryBuilder<M>) -> FusionPlan`：静态分析 WHERE/排序，识别可下推缓存/搜索子句
- [ ] M5-T2.2 实现 `FusionExecutor::execute(plan) -> Result<Vec<Row>>`：主库查询 + 缓存/搜索并行执行 + 聚合
- [ ] M5-T2.3 单元测试：`where_eq` + 缓存键命中 → 主库跳过
- [ ] M5-T2.4 单元测试：主库失败回退（缓存可读）→ 返回缓存结果 + 降级标记

## M5-T3：数据同步 + 实验评估

**子任务**：
- [ ] M5-T3.1 复用既有 CDC（`packages/sz-orm-queue/src/cdc/capturer.rs:12`）做主库→缓存/搜索索引同步
- [ ] M5-T3.2 编写《db-fusion 实验评估报告》：POC 结果 + 价值判断（转正/废弃建议）
- [ ] M5-T3.3 门禁验证 + 默认 feature 行为不变

**验收标准**：POC 可运行；评估报告给出转正/废弃建议；默认行为不变

**依赖**：无（独立实验）

---

# 十、M6：最终验证与文档同步（全局，0.5 周）

## M6-T1：14 道门禁全量验证

**子任务**：
- [ ] M6-T1.1 `cargo fmt --all -- --check`（门禁 1）
- [ ] M6-T1.2 `cargo check --workspace --all-targets`（门禁 2）
- [ ] M6-T1.3 `cargo clippy --workspace --all-targets -- -D warnings`（门禁 3）
- [ ] M6-T1.4 `cargo test --workspace -j 2 --no-fail-fast`（门禁 4，v4.2.0 基线不回退）
- [ ] M6-T1.5 `cargo doc --workspace --no-deps --all-features`（门禁 5）
- [ ] M6-T1.6 `cargo audit` + `cargo deny check`（门禁 6）
- [ ] M6-T1.7 `cargo test --workspace -- --ignored`（门禁 7）
- [ ] M6-T1.8 扫描占位实现（门禁 8）
- [ ] M6-T1.9 `scripts/check-sql-injection.ps1`（门禁 9）
- [ ] M6-T1.10 `cargo check --workspace --all-targets --all-features`（门禁 10）
- [ ] M6-T1.11 `git diff --name-only HEAD`（门禁 11，ADR-0001）
- [ ] M6-T1.12 `python scripts/check-doc-consistency.py`（门禁 12）
- [ ] M6-T1.13 `bash scripts/audit-verify.sh <审计报告.md>`（门禁 13）
- [ ] M6-T1.14 `python scripts/check-doc-sync.py --diff HEAD`（门禁 14）
...
- [ ] M6-T2.3 更新 `CHANGELOG.md` / `README.md` / `AGENTS.md`（新包 + feature 列表）
- [ ] M6-T2.4 验证 sz-pay 兼容性（不启用新 feature 行为不变）
- [ ] M6-T2.5 更新 `docs/评估/2026-08-12_v4.2.0_基线评估.md` → 追加 v4.3.0 章节

## M6-T3：feature gate 逐步启用计划

**子任务**：
- [ ] M6-T3.1 每个 feature 独立编译验证
- [ ] M6-T3.2 feature 组合编译验证（含与 v4.2.0 7 个 feature 的组合）
- [ ] M6-T3.3 全 feature 编译验证
- [ ] M6-T3.4 制定逐步启用计划（按 P1→P2→P3）

**验收标准**：14 道门禁全部通过；文档同步；sz-pay 兼容；feature 组合无冲突
...
```
M0（文档，并行）      M1-T1 → M1-T2 → M1-T3 → M1-T6
                      M1-T4 ──────────↗          │
                      M1-T5 ────────────────────↗
M2（v4.2.0 后）      M2-T1 → M2-T2 → M2-T3
M3（并行）            M3-T1 → M3-T2 → M3-T5
                      M3-T3 → M3-T4 →↗
M4（v4.2.0 后）      M4-T1 → M4-T2 → M4-T3
M5（可选实验）        M5-T1 → M5-T2 → M5-T3
M6（最终）           M6-T1 ← M1-T6 / M2-T3 / M3-T5 / M4-T3 / M5-T3
                      M6-T2 ← M6-T1
                      M6-T3 ← M6-T1
```
...
|---------|---------|---------|---------|
| R-001 | EXPLAIN 输出格式随 DB 版本变化解析失败 | 解析失败降级为无警告（不阻断编译），Parser 按方言版本适配 | M1-T2/M1-T3 |
| R-002 | 编译期警告过度干扰开发者 | 默认 warning 非阻断，提供 `SZ_ORM_EXPLAIN_ROW_THRESHOLD` 配置与 allow 属性 | M1-T3 |
| R-003 | N+1 静态检测误报/漏报 | 检测模式保守（仅循环体内直接查询调用），标注宏 + 批量扫描双入口，运行时 `N1QueryDetector` 交叉验证 | M2-T1/M2-T3 |
| R-004 | `cli/src/main.rs` 与 v4.2.0 修改冲突 | M2 排期在 v4.2.0 交付后，merge 时 `git diff` 先行核对 | M2-T2 |
| R-005 | `sz-orm-macros` 扩展影响既有 `query!` 行为 | 全部新增逻辑在 `db-verify` feature 内，默认模式零改动；既有测试基线验证 | M1-T3 |
| R-006 | 自适应查询自动切缓存导致脏读 | 缓存 TTL 可配 + 默认关闭自动缓存（需显式开启），统计决策仅建议不强制 | M4-T2 |
| R-007 | 编译期治理 compile_error 阻断既有代码编译 | 治理检查仅在 `compile-governance` feature 启用时生效，默认关闭 | M3-T3 |
| R-008 | db-fusion 拆分语义不透明 | 实验阶段仅支持可证明安全的拆分（主键等值 + 缓存键），评估报告决定转正/废弃 | M5-T2 |
| R-009 | 新增 feature 与 v4.2.0 7 个 feature 组合编译失败 | 门禁 10 全组合编译 + M6-T3 组合验证 | M6-T1/M6-T3 |
...
| M1 | 5 方言 EXPLAIN 解析 + 编译期警告 + 计划回归 + 火焰图（Brendan Gregg 兼容） |
| M2 | 3 种 N+1 模式检测；CLI 批量扫描；与运行时 N1QueryDetector 交叉一致 |
| M3 | Mermaid/Graphviz/HTML 血缘导出 + 影响分析；PII 编译期强制；合规报告 |
| M4 | 统计采集 <1μs；自动分页/缓存决策；复用既有实现 |
| M5 | db-fusion POC + 转正/废弃评估报告 |
| M6 | 14 道门禁 + 文档同步 + sz-pay 兼容 + feature 组合验证 |
...
1. **立即启动（与 v4.2.0 并行）**：M0（0.5 周，纯文档）→ M1（sz-orm-explain 先做，flamegraph 建议 v4.2.0 M1 后）→ M3（sz-orm-audit 扩展）
2. **v4.2.0 交付后启动**：M2（cli 同文件）→ M4（依赖 core 最终形态）→ M5（可选实验）
3. **最后**：M6 最终验证（14 道门禁 + 文档 + 版本号）
...
- M6-T2 统一更新 API-STABILITY.md/README/AGENTS/engineering-practices + 评估文档
- 版本号 v4.2.0 → v4.3.0 在 M6-T2 更新

---

> 文档结束。本规划所有 file:line 证据均来自 2026-08-12 实测（非文档声明），与 v4.2.0 边界清晰（唯一同文件风险 `cli/src/main.rs` 已通过排期规避），遵循 AGENTS.md 审计合规铁律。
