...
> 更新日期：2026-07-21
> 数据来源：criterion 基准（benches/core_bench.rs，sample_size=10, measurement_time=3s, warm_up=1s）+ stress 压力测试 + 真实 DB 超大数据量集成测试（本机 + 远程云 122.51.216.76）+ 1h Soak Test 实测 + cargo audit/cargo deny 安全审计
...
| PostgreSQL（远程云） | 10 万行批量 INSERT | **4.11 万行/s** | 0.28× | 122.51.216.76:5432 |
| MySQL 8.x（远程云） | 10 万行批量 INSERT | **2.57 万行/s** | 0.18× | 122.51.216.76:8802 |
...
| 真实云 DB Jepsen | MySQL 5 项 + PG 5 项（122.51.216.76） | ✅ 10/10 通过 |
...
| 数据库 | URL / 环境变量 |
|--------|-----|
| MySQL 9.6（本机） | `mysql://root:test123@127.0.0.1:3306/sz_orm_test` |
| PostgreSQL 18（本机） | `postgres://postgres:test123@127.0.0.1:5432/sz_orm_test` |
| Oracle 23ai Free（本机） | `Connector::new("sys","test123","127.0.0.1:1521/freepdb1.FALSE").privilege(Sysdba)` |
| MySQL 8.x（远程云） | `SZ_ORM_MYSQL_URL=mysql://root:0167df3598924d19@122.51.216.76:8802/shop` |
| PostgreSQL（远程云） | `SZ_ORM_PG_URL=postgres://lewuli:JkbC2jsaWAYDe2Gz@122.51.216.76:5432/lewuli` |
...
| PostgreSQL | 18 | `E:\db\pgsql\`，数据目录 `E:\db\pgsql18-data`，端口 5432 |
...
**总体结论**：✅ **0 个未忽略漏洞**（10 个已知漏洞全部通过 `--ignore` 忽略并记录原因）

| RUSTSEC ID | 严重程度 | 受影响 crate | 来源 | 忽略原因 |
|------------|---------|-------------|------|---------|
| RUSTSEC-2023-0071 | 5.9 medium | rsa 0.9.10 | sqlx-mysql 0.8.6 传递依赖 | 同主机时序侧信道，服务端可控 |
| RUSTSEC-2026-0049 | - | rustls-webpki 0.102.8 | sz-orm-mqtt real-broker feature | 上游 0.103 不兼容；仅影响 real-broker |
| RUSTSEC-2026-0098 | - | rustls-webpki 0.102.8 | 同上 | 同上 |
| RUSTSEC-2026-0099 | - | rustls-webpki 0.102.8 | 同上 | 同上 |
| RUSTSEC-2026-0104 | - | rustls-webpki 0.102.8 | 同上 | 同上 |
| RUSTSEC-2026-0194 | 7.5 high | quick-xml 0.38.4 | sz-orm-queue rabbitmq feature | 服务端可控输入；上游 lapin 未升级 |
| RUSTSEC-2026-0195 | 7.5 high | quick-xml 0.38.4 | 同上 | 同上 |
| RUSTSEC-2025-0134 | unmaintained | rustls-pemfile 2.2.0 | sz-orm-mqtt real-broker feature | 仅影响 real-broker；rustls 0.23 内置替代 |

**未维护警告**（1 项，无法绕过）：
- `paste 1.0.15`（RUSTSEC-2024-0436）— 来自 sqlx 0.8 传递依赖，无可用替代
...
| 检查项 | 结果 | 说明 |
|--------|------|------|
| `advisories` | ✅ ok | 8 个忽略项与 cargo audit 一致 |
...
- **零未忽略漏洞**：10 个 RUSTSEC 公告全部为传递依赖（sqlx/rumqttc/lapin），已记录详细忽略原因
...
2. 启用 `sz-orm-queue` 的 `rabbitmq` feature 前，评估 RUSTSEC-2026-0194/0195（quick-xml 漏洞）
3. 关注 sqlx 0.9+ 发布，自动消除 rsa Marvin Attack 公告
