...
use sz_orm_core::zero_copy_pipeline::{ZeroCopyPipeline, ZeroCopyTypeRegistry};
...
fn main() {
    ...
    println!(
        "[PerfConfig] simd={}, zero_copy={}, prewarm={}, plan_cache={}",
        config.simd_enabled,
        config.zero_copy_enabled,
        config.prewarm_enabled,
        config.plan_cache_enabled
    );
    ...
    row.insert(
        "name".to_string(),
        sz_orm_core::Value::String("Alice".into()),
    );
    ...
    println!(
        "[计划缓存] SQL=\"{}\", MySQL指纹={}, PG指纹={}, 不同类型不同指纹={}",
        sql,
        fp1,
        fp2,
        fp1 != fp2
    ...
    println!(
        "  simd_latency_reduction_pct={:.1}%",
        snap.simd_latency_reduction_pct
    );
    println!("  zero_copy_hit_count={}", snap.zero_copy_hit_count);
    println!(
        "  zero_copy_rss_reduction_pct={:.1}%",
        snap.zero_copy_rss_reduction_pct
    );
    println!("  prewarm_success_count={}", snap.prewarm_success_count);
    println!("  plan_cache_hit_rate={:.2}", snap.plan_cache_hit_rate);
    println!(
        "  plan_cache_eviction_count={}",
        snap.plan_cache_eviction_count
    );
    ...
    assert!(
        default_config.plan_cache_enabled,
        "默认计划缓存开启保持既有行为"
    );
