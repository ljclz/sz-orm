...
//! 检测到 SQL 注入特征（单引号奇数）时主动 panic 触发 crash 报告。
...
fuzz_target!(|data: &[u8]| {
    ...
    if quote_count % 2 != 0 {
        // 转义后单引号为奇数 = 潜在注入点，主动 panic 让 libfuzzer 报告
        panic!(
            "Potential SQL injection: unpaired quote in {owned:?} from {val:?} (quote_count={quote_count})"
        );
    }
